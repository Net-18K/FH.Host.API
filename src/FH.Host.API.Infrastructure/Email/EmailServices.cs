...
namespace FH.Host.API.Infrastructure.Email
{
    ...
    public class EmailServices : IEmailServices
    {
        ...
        public string SendQQEmail(string recipientId, string recipientBody)
        {
            ...
            // 邮件内容
            mailMessage.Body = recipientBody;
            // 实例化一个SmtpClient类
        ...
        /// </summary>
        /// <param name="RecipientId">收件人邮箱账号</param>
        public SendRandomCodeResultDto SendRandomCodeQQEmail(string recipientId)
        {
            // 生成验证码
            string randomCode = RandomCode();
            string recipientBody = string.Format("【{0}】您的邮箱验证码是：{1}。请在页面中提交验证码完成验证。{2}", AppConfigurtaionService.Configuration["ProjectInfo:Project_Name"], randomCode, string.Format(MailContent, DateTime.Now.ToString()));

            // 返回邮件验证码信息
            return new SendRandomCodeResultDto()
            {
                RecipientId = recipientId,
                RecipientBody = SendQQEmail(recipientId, recipientBody),// 发送邮件
